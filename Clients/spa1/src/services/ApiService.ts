...
export class ApiService {
  ...
  public postBook(book: any): Promise<any> {
    return this.authService.getUser().then(user => {
      if (user && user.access_token) {
        return this._postBook(user.access_token, book).catch(error => {
          if (error.response.status === 401) {
            return this.authService.renewToken().then(renewedUser => {
              return this._postBook(renewedUser.access_token, book);
            });
          }
          throw error;
        });
  ...
  private _postBook(token: string, book: any) {
    const headers = {
      Accept: 'application/json',
      Authorization: 'Bearer ' + token
    };
